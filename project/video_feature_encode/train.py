...
train_data_file = r"E:\Data\MLData\视觉特征编码\train\train.csv"
def train(args):
    """训练模型  
        1、创建unet模型，并绑定设备
        2、加载数据，使用batch
    ...
    video_feature_dataset = VideoFeatureDataset(train_data_file, mode=args.action)
    dataloaders = DataLoader(video_feature_dataset, batch_size=batch_size, shuffle=True, num_workers=4)  # 使用pytorch的数据加载函数加载数据
