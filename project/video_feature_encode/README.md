...
在优化器中添加权重衰减超参数，weight_decay；droupOut层，测试需要关闭droupOut层，调用net.eval()即可。
  - 10.1 设置权重衰减，0.0001
  - 10.2 增加归一化层。lenet中增加了bn归一化层，模型较为稳定。
...
- 15、尝试其他人的baseline
  - 15.1 首先采用仅三层全连接层，最后一个全连接输出层，测试集精度即可达到0.82；没有做任何数据增强。训练的loss一直在下降，精度不在提升，可能过拟合了。尝试增加dropout层。
  - 15.2 每个数据取50帧为一组进行训练,用baseline1全连接层训练效果不佳。
...
- 17、如何训练一个优秀的模型
  ...
  - 2）然后，去抑制过拟合：(1) dropout。(2）正则化。(3)图像增强（抑制过拟合的最佳方法是通过增加训练数据或者缩小网络的规模。这里所说的方法是没有办法增加训练数据的话）
  - 3）再次，调节超参数：（1）学习速率、（2）隐藏层单元数、（3）训练轮次。
