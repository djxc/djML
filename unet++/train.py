...
from utils.dataset import BasicDataset, RSDataset
...
def train_net(net, cfg):
    ...
    train_dataset = RSDataset(
        cfg.images_dir, cfg.masks_dir, cfg.train_file, cfg.scale)
    # 加载验证数据
    val_dataset = RSDataset(
        cfg.images_dir, cfg.masks_dir, cfg.val_file, cfg.scale)
    ...
    for epoch in range(cfg.epochs):
        net.train()
        epoch_loss = 0
        newLR = optimizer.state_dict()['param_groups'][0]['lr']
            ...
            for batch in train_loader:
                ...
                if cfg.deepsupervision:
                    ...
                    for inference_mask in inferences:
                        # print(inference_mask.shape, masks.shape, len(inferences))
                ...
                img_size += cfg.batch_size
                writer.add_scalar('Loss/train', loss.item(), global_step)
                writer.add_scalar(
                    'model/lr', optimizer.param_groups[0]['lr'], global_step)
                # print(epoch_loss/img_size)
                pbar.set_postfix(**{'loss': epoch_loss/img_size})
                ...
                if global_step % (len(train_dataset) // (5 * cfg.batch_size)) == 0:
        ...
        print("epoch_loss: ", epoch_loss/n_train, "; learning rate: ", newLR)
        if cfg.save_cp:
            try:
            ...
            ckpt_name = '1015_rs_epoch_' + str(epoch + 1) + '.pth'
            ...
            if cfg.deepsupervision:
                ...
                for masks_pred in masks_preds:
                    ...
                    for true_mask, pred in zip(true_masks, masks_pred):
                        ...
                        if cfg.n_classes > 1:
                            # print(pred.shape, true_mask.shape)
                            # print(pred.unsqueeze(dim=0).shape,
                            #       true_mask.unsqueeze(dim=0).shape)
