...
from data import BananasDataset
...
CURRENT_IMAGE_PATH = r"D:\Data\MLData\MLData\detection\banana-detection"

def load_data_bananas(batch_size):
    """加载香蕉检测数据集。"""
    train_iter = torch.utils.data.DataLoader(BananasDataset(is_train=True, workspace=CURRENT_IMAGE_PATH),
                                             batch_size, shuffle=True)
    val_iter = torch.utils.data.DataLoader(BananasDataset(is_train=False, workspace=CURRENT_IMAGE_PATH),
                                           batch_size)
...
if __name__ == "__main__":
    img_path = os.path.join(CURRENT_IMAGE_PATH, "bananas_train", "images", "0.png")
